...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleAddComponentToBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    NamesToTry.Add(ComponentType);
    if (!ComponentType.EndsWith(TEXT("Component")))
    {
        NamesToTry.Add(ComponentType + TEXT("Component"));
    }
    if (!ComponentType.StartsWith(TEXT("U")))
    {
        NamesToTry.Add(TEXT("U") + ComponentType);
        if (!ComponentType.EndsWith(TEXT("Component")))
        {
            NamesToTry.Add(TEXT("U") + ComponentType + TEXT("Component"));
        }
    }
    for (TObjectIterator<UClass> It; It && !ComponentClass; ++It)
    {
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCompileBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    ResultObj->SetNumberField(TEXT("blueprint_status"), static_cast<int32>(Blueprint->Status));
    if (!bCompiled) ResultObj->SetStringField(TEXT("error"), TEXT("Blueprint compilation failed. Inspect the editor compiler log."));
    return ResultObj;
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleSetBlueprintProperty(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (Params->HasField(TEXT("property_value")))
    {
        ...
        {
            // Mark as modified and compile so CDO is up-to-date
            FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);
            FKismetEditorUtilities::CompileBlueprint(Blueprint);
