...
class GroundedTest:
 def __init__(self):
  ...
  self.cases=[('stop',walk,duration) for walk in [True,False] for duration in [.37,.61,.89,1.14]]+[('turn',False,90),('turn',False,-90),('interrupt',False,90),('fall',False,0),('teleport',False,0)]
 ...
 def tick(self,dt):
  try:
   if self.i>=len(self.cases):
    ...
    Path('D:/CodexUE55Lab/grounded-test.json').write_text(json.dumps({'results':self.results,'frames':self.rows},indent=2))
   ...
   if t>duration:
    ...
    Path('D:/CodexUE55Lab/grounded-progress.json').write_text(json.dumps(self.results,indent=2));self.next()
  except Exception:
