...
def step(dt):
 ...
 try:
  if idx>=len(cases):
   ...
   Path('D:/CodexUE55Lab/grounded-directions.json').write_text(json.dumps(results,indent=2))
  ...
  if unreal.GameplayStatics.get_time_seconds(w)-elapsed>=2.0:
   ...
   idx+=1;setup=True
   Path("D:/CodexUE55Lab/grounded-directions-progress.json").write_text(json.dumps(results,indent=2))
