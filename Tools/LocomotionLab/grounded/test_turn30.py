...
class GroundedTest:
 def __init__(self):
  ...
  self.cases=[('turn',False,90),('turn',False,-90)]
 ...
 def tick(self,dt):
  try:
   if self.i>=len(self.cases):
    ...
    Path('D:/CodexUE55Lab/grounded-turn30.json').write_text(json.dumps({'results':self.results,'frames':self.rows},indent=2))
    unreal.SystemLibrary.execute_console_command(self.w,'t.MaxFPS '+str(self.oldfps));unreal.unregister_slate_post_tick_callback(self.handle);print('GROUNDED_TEST_DONE');return
   ...
   if t>duration:
    ...
    Path('D:/CodexUE55Lab/grounded-turn30-progress.json').write_text(json.dumps(self.results,indent=2));self.next()
  except Exception:
...
grounded_test.oldfps=unreal.SystemLibrary.get_console_variable_float_value('t.MaxFPS')
unreal.SystemLibrary.execute_console_command(grounded_test.w,'t.MaxFPS 30')
