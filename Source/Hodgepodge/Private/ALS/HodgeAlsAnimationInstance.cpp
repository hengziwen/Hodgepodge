...
#include "ALS/Utility/HodgeAlsPrivateMemberAccessor.h"
#include "ALS/Utility/HodgeAlsRotation.h"
#include "ALS/Utility/HodgeAlsUtility.h"
#include "ALS/Utility/HodgeAlsVector.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HodgeAlsAnimationInstance)

HODGEALS_DEFINE_PRIVATE_MEMBER_ACCESSOR(HodgeAlsGetAnimationCurvesAccessor, &FAnimInstanceProxy::GetAnimationCurves,
                                   const TMap<FName, float>& (FAnimInstanceProxy::*)(EAnimCurveType) const)

void UHodgeAlsAnimationInstance::NativeInitializeAnimation()
{
	...
	if (IsValid(Mesh) && IsValid(Mesh->GetSkinnedAsset()))
	{
		...
		GetThighAxis(ReferenceSkeleton, PelvisBoneIndex, UHodgeAlsConstants::FootLeftBoneName(), FeetState.Left.ThighAxis);
		GetThighAxis(ReferenceSkeleton, PelvisBoneIndex, UHodgeAlsConstants::FootRightBoneName(), FeetState.Right.ThighAxis);
	}
}

void UHodgeAlsAnimationInstance::NativeBeginPlay()
{
...
void UHodgeAlsAnimationInstance::NativeUpdateAnimation(const float DeltaTime)
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::NativeUpdateAnimation"),
	                            STAT_UHodgeAlsAnimationInstance_NativeUpdateAnimation, STATGROUP_HodgeAls)
	...
	bDisplayDebugTraces = UHodgeAlsDebugUtility::ShouldDisplayDebugForActor(Character, UHodgeAlsConstants::TracesDebugDisplayName());
#endif
...
void UHodgeAlsAnimationInstance::NativeThreadSafeUpdateAnimation(const float DeltaTime)
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::NativeThreadSafeUpdateAnimation"),
	                            STAT_UHodgeAlsAnimationInstance_NativeThreadSafeUpdateAnimation, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
...
void UHodgeAlsAnimationInstance::NativePostUpdateAnimation()
{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::NativePostUpdateAnimation"),
	                            STAT_UHodgeAlsAnimationInstance_NativePostUpdateAnimation, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
...
FAnimInstanceProxy* UHodgeAlsAnimationInstance::CreateAnimInstanceProxy()
{
	return new FHodgeAlsAnimationInstanceProxy{this};
}

FHodgeAlsControlRigInput UHodgeAlsAnimationInstance::GetControlRigInput() const
{
	return {
		...
		.bFootOffsetAllowed = LocomotionMode != HodgeAlsLocomotionModeTags::InAir,
...
void UHodgeAlsAnimationInstance::RefreshMovementBaseOnGameThread()
{
...
void UHodgeAlsAnimationInstance::RefreshLayering()
{
	const auto& Curves{
		HodgeAlsGetAnimationCurvesAccessor::Access(GetProxyOnAnyThread<FAnimInstanceProxy>(), EAnimCurveType::AttributeCurve)
	};
	...
	LayeringState.HeadBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerHeadCurveName());
	LayeringState.HeadAdditiveBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerHeadAdditiveCurveName());
	LayeringState.HeadSlotBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerHeadSlotCurveName());

	// The mesh space blend will always be 1 unless the local space blend is 1.

	LayeringState.ArmLeftBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmLeftCurveName());
	LayeringState.ArmLeftAdditiveBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmLeftAdditiveCurveName());
	LayeringState.ArmLeftSlotBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmLeftSlotCurveName());
	LayeringState.ArmLeftLocalSpaceBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmLeftLocalSpaceCurveName());
	LayeringState.ArmLeftMeshSpaceBlendAmount = !FAnimWeight::IsFullWeight(LayeringState.ArmLeftLocalSpaceBlendAmount);

	// The mesh space blend will always be 1 unless the local space blend is 1.

	LayeringState.ArmRightBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmRightCurveName());
	LayeringState.ArmRightAdditiveBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmRightAdditiveCurveName());
	LayeringState.ArmRightSlotBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmRightSlotCurveName());
	LayeringState.ArmRightLocalSpaceBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerArmRightLocalSpaceCurveName());
	LayeringState.ArmRightMeshSpaceBlendAmount = !FAnimWeight::IsFullWeight(LayeringState.ArmRightLocalSpaceBlendAmount);

	LayeringState.HandLeftBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerHandLeftCurveName());
	LayeringState.HandRightBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerHandRightCurveName());

	LayeringState.SpineBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerSpineCurveName());
	LayeringState.SpineAdditiveBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerSpineAdditiveCurveName());
	LayeringState.SpineSlotBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerSpineSlotCurveName());

	LayeringState.PelvisBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerPelvisCurveName());
	LayeringState.PelvisSlotBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerPelvisSlotCurveName());

	LayeringState.LegsBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerLegsCurveName());
	LayeringState.LegsSlotBlendAmount = GetCurveValue(Curves, UHodgeAlsConstants::LayerLegsSlotCurveName());
}

void UHodgeAlsAnimationInstance::RefreshPose()
{
	const auto& Curves{
		HodgeAlsGetAnimationCurvesAccessor::Access(GetProxyOnAnyThread<FAnimInstanceProxy>(), EAnimCurveType::AttributeCurve)
	};
	...
	PoseState.GroundedAmount = GetCurveValue(Curves, UHodgeAlsConstants::PoseGroundedCurveName());
	PoseState.InAirAmount = GetCurveValue(Curves, UHodgeAlsConstants::PoseInAirCurveName());

	PoseState.StandingAmount = GetCurveValue(Curves, UHodgeAlsConstants::PoseStandingCurveName());
	PoseState.CrouchingAmount = GetCurveValue(Curves, UHodgeAlsConstants::PoseCrouchingCurveName());

	PoseState.MovingAmount = GetCurveValue(Curves, UHodgeAlsConstants::PoseMovingCurveName());

	PoseState.GaitAmount = FMath::Clamp(GetCurveValue(Curves, UHodgeAlsConstants::PoseGaitCurveName()), 0.0f, 3.0f);
	PoseState.GaitWalkingAmount = UHodgeAlsMath::Clamp01(PoseState.GaitAmount);
	PoseState.GaitRunningAmount = UHodgeAlsMath::Clamp01(PoseState.GaitAmount - 1.0f);
	PoseState.GaitSprintingAmount = UHodgeAlsMath::Clamp01(PoseState.GaitAmount - 2.0f);
	...
	PoseState.UnweightedGaitWalkingAmount = UHodgeAlsMath::Clamp01(PoseState.UnweightedGaitAmount);
	PoseState.UnweightedGaitRunningAmount = UHodgeAlsMath::Clamp01(PoseState.UnweightedGaitAmount - 1.0f);
	PoseState.UnweightedGaitSprintingAmount = UHodgeAlsMath::Clamp01(PoseState.UnweightedGaitAmount - 2.0f);
}

void UHodgeAlsAnimationInstance::RefreshViewOnGameThread()
{
...
void UHodgeAlsAnimationInstance::RefreshView(const float DeltaTime)
{
	...
	const auto ViewAmount{1.0f - GetCurveValueClamped01(UHodgeAlsConstants::ViewBlockCurveName())};
	const auto AimingAmount{GetCurveValueClamped01(UHodgeAlsConstants::AllowAimingCurveName())};
...
bool UHodgeAlsAnimationInstance::IsSpineRotationAllowed()
{
	return RotationMode == HodgeAlsRotationModeTags::Aiming;
}

void UHodgeAlsAnimationInstance::RefreshSpine(const float SpineBlendAmount, const float DeltaTime)
{
	...
	if (SpineState.bSpineRotationAllowed)
	{
		...
		else
		{
			...
			SpineState.SpineAmount = UHodgeAlsMath::ExponentialDecay(SpineState.SpineAmount, 1.0f, DeltaTime, InterpolationSpeed);

			SpineState.CurrentYawAngle = UHodgeAlsRotation::LerpAngle(SpineState.LastYawAngle, ViewState.YawAngle,
	...
	else
	{
		...
		else
		{
			...
			SpineState.SpineAmount = UHodgeAlsMath::ExponentialDecay(SpineState.SpineAmount, 0.0f, DeltaTime,
			...
			SpineState.CurrentYawAngle = UHodgeAlsRotation::LerpAngle(0.0f, SpineState.LastYawAngle + YawAngleOffset,
			                                                     SpineState.SpineAmount * SpineState.SpineAmountScale +
			                                                     SpineState.SpineAmountBias);
		}
	}

	SpineState.YawAngle = UHodgeAlsRotation::LerpAngle(0.0f, SpineState.CurrentYawAngle, SpineBlendAmount);
}

void UHodgeAlsAnimationInstance::InitializeLook()
{
	LookState.bInitializationRequired = true;
}

void UHodgeAlsAnimationInstance::RefreshLook()
{
#if WITH_EDITOR
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshLook"), STAT_UHodgeAlsAnimationInstance_RefreshLook, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
	...
	if (RotationMode == HodgeAlsRotationModeTags::VelocityDirection)
	{
	...
	else
	{
		...
		if (DeltaYawAngle > 180.0f - UHodgeAlsRotation::CounterClockwiseRotationAngleThreshold)
		{
		...
		const auto InterpolationAmount{UHodgeAlsMath::ExponentialDecay(GetDeltaSeconds(), InterpolationSpeed)};

		LookState.YawAngle = FMath::UnwindDegrees(YawAngle + DeltaYawAngle * InterpolationAmount);
		LookState.PitchAngle = UHodgeAlsRotation::LerpAngle(LookState.PitchAngle, TargetPitchAngle, InterpolationAmount);
	}
...
void UHodgeAlsAnimationInstance::RefreshLocomotionOnGameThread()
{
	check(IsInGameThread())
...
void UHodgeAlsAnimationInstance::InitializeLean()
{
	LeanState.RightAmount = 0.0f;
	LeanState.ForwardAmount = 0.0f;
}

void UHodgeAlsAnimationInstance::InitializeGrounded()
{
	GroundedState.VelocityBlend.bInitializationRequired = true;
}

void UHodgeAlsAnimationInstance::RefreshGrounded()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshGrounded"), STAT_UHodgeAlsAnimationInstance_RefreshGrounded, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
...
FVector3f UHodgeAlsAnimationInstance::GetRelativeVelocity() const
{
	return FVector3f{LocomotionState.RotationQuaternion.UnrotateVector(LocomotionState.Velocity)};
}

FVector2f UHodgeAlsAnimationInstance::GetRelativeAccelerationAmount() const
{
	...
	return FVector2f{UHodgeAlsVector::ClampMagnitude01(RelativeAcceleration / MaxAcceleration)};
}

void UHodgeAlsAnimationInstance::RefreshVelocityBlend()
{
	...
	if (VelocityBlend.bInitializationRequired || Settings->Grounded.VelocityBlendInterpolationSpeed <= 0.0f)
	{
		...
		VelocityBlend.ForwardAmount = UHodgeAlsMath::Clamp01(TargetVelocityBlend.X);
		...
		VelocityBlend.RightAmount = UHodgeAlsMath::Clamp01(TargetVelocityBlend.Y);
	}
	else
	{
		// WWe use UHodgeAlsMath::ExponentialDecay() instead of FMath::FInterpTo(), because FMath::FInterpTo() is very sensitive to large
		// delta time, at low FPS interpolation becomes almost instant which causes issues with character pose during the stop.

		const auto InterpolationAmount{UHodgeAlsMath::ExponentialDecay(GetDeltaSeconds(), Settings->Grounded.VelocityBlendInterpolationSpeed)};

		VelocityBlend.ForwardAmount = FMath::Lerp(VelocityBlend.ForwardAmount,
		                                          UHodgeAlsMath::Clamp01(TargetVelocityBlend.X),
		                                          InterpolationAmount);
		...
		VelocityBlend.RightAmount = FMath::Lerp(VelocityBlend.RightAmount,
		                                        UHodgeAlsMath::Clamp01(TargetVelocityBlend.Y),
		                                        InterpolationAmount);
	}
}

void UHodgeAlsAnimationInstance::RefreshGroundedLean()
{
	...
	{
		const auto InterpolationAmount{UHodgeAlsMath::ExponentialDecay(GetDeltaSeconds(), Settings->General.LeanInterpolationSpeed)};
...
void UHodgeAlsAnimationInstance::RefreshGroundedMovement()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshGroundedMovement"),
	                            STAT_UHodgeAlsAnimationInstance_RefreshGroundedMovement, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
	...
	GroundedState.HipsDirectionLockAmount = FMath::Clamp(GetCurveValue(UHodgeAlsConstants::HipsDirectionLockCurveName()), -1.0f, 1.0f);
...
void UHodgeAlsAnimationInstance::RefreshMovementDirection(const float ViewRelativeVelocityYawAngle)
{
	...
	if (RotationMode == HodgeAlsRotationModeTags::VelocityDirection || Gait == HodgeAlsGaitTags::Sprinting)
	{
		GroundedState.MovementDirection = EHodgeAlsMovementDirection::Forward;
		return;
	}
	...
	GroundedState.MovementDirection = UHodgeAlsMath::CalculateMovementDirection(
		ViewRelativeVelocityYawAngle, ForwardHalfAngle, AngleThreshold);
}

void UHodgeAlsAnimationInstance::RefreshRotationYawOffsets(const float ViewRelativeVelocityYawAngle)
{
...
void UHodgeAlsAnimationInstance::InitializeStandingMovement()
{
	StandingState.SprintTime = 0.0f;
	StandingState.bPivotActive = false;
}

void UHodgeAlsAnimationInstance::RefreshStandingMovement()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshStandingMovement"),
	                            STAT_UHodgeAlsAnimationInstance_RefreshStandingMovement, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
	...
	StandingState.WalkRunBlendAmount = Gait == HodgeAlsGaitTags::Walking ? 0.0f : 1.0f;
	...
	StandingState.SprintBlockAmount = GetCurveValueClamped01(UHodgeAlsConstants::SprintBlockCurveName());

	if (Gait != HodgeAlsGaitTags::Sprinting)
	{
		StandingState.SprintTime = 0.0f;
...
void UHodgeAlsAnimationInstance::ActivatePivot()
{
	StandingState.bPivotActive = LocomotionState.Speed < Settings->Standing.PivotActivationSpeedThreshold;
}

void UHodgeAlsAnimationInstance::RefreshCrouchingMovement()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshCrouchingMovement"),
	                            STAT_UHodgeAlsAnimationInstance_RefreshCrouchingMovement, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
...
void UHodgeAlsAnimationInstance::RefreshInAirOnGameThread()
{
	check(IsInGameThread())
...
void UHodgeAlsAnimationInstance::RefreshInAir()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshInAir"), STAT_UHodgeAlsAnimationInstance_RefreshInAir, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
	...
	if (InAirState.bJumped)
	{
		...
		InAirState.JumpPlayRate = UHodgeAlsMath::LerpClamped(MinPlayRate, MaxPlayRate, LocomotionState.Speed / ReferenceSpeed);
	}
...
void UHodgeAlsAnimationInstance::RefreshGroundPrediction()
{
	...
	const auto AllowanceAmount{1.0f - GetCurveValueClamped01(UHodgeAlsConstants::GroundPredictionBlockCurveName())};
	...
	if (bDisplayDebugTraces)
	{
		if (IsInGameThread())
		{
			UHodgeAlsDebugUtility::DrawSweepSingleCapsule(GetWorld(), Hit.TraceStart, Hit.TraceEnd, FRotator::ZeroRotator,
...
void UHodgeAlsAnimationInstance::RefreshInAirLean()
{
...
void UHodgeAlsAnimationInstance::RefreshFeetOnGameThread()
{
	check(IsInGameThread())

	const auto* Mesh{GetSkelMeshComponent()};

	FeetState.PelvisRotation = FQuat4f{Mesh->GetSocketTransform(UHodgeAlsConstants::PelvisBoneName(), RTS_Component).GetRotation()};

	const auto FootLeftTargetTransform{
		Mesh->GetSocketTransform(Settings->General.bUseFootIkBones
			                         ? UHodgeAlsConstants::FootLeftIkBoneName()
			                         : UHodgeAlsConstants::FootLeftVirtualBoneName())
	};
	...
	const auto FootRightTargetTransform{
		Mesh->GetSocketTransform(Settings->General.bUseFootIkBones
			                         ? UHodgeAlsConstants::FootRightIkBoneName()
			                         : UHodgeAlsConstants::FootRightVirtualBoneName())
	};
...
void UHodgeAlsAnimationInstance::RefreshFeet(const float DeltaTime)
{
	FeetState.FootPlantedAmount = FMath::Clamp(GetCurveValue(UHodgeAlsConstants::FootPlantedCurveName()), -1.0f, 1.0f);
	FeetState.FeetCrossingAmount = GetCurveValueClamped01(UHodgeAlsConstants::FeetCrossingCurveName());

	const auto ComponentTransformInverse{GetProxyOnAnyThread<FAnimInstanceProxy>().GetComponentTransform().Inverse()};

	RefreshFoot(FeetState.Left, UHodgeAlsConstants::FootLeftIkCurveName(),
	            UHodgeAlsConstants::FootLeftLockCurveName(), ComponentTransformInverse, DeltaTime);

	RefreshFoot(FeetState.Right, UHodgeAlsConstants::FootRightIkCurveName(),
	            UHodgeAlsConstants::FootRightLockCurveName(), ComponentTransformInverse, DeltaTime);
}

void UHodgeAlsAnimationInstance::RefreshFoot(FHodgeAlsFootState& FootState, const FName& IkCurveName, const FName& LockCurveName,
...
void UHodgeAlsAnimationInstance::ProcessFootLockTeleport(const float IkAmount, FHodgeAlsFootState& FootState) const
{
...
void UHodgeAlsAnimationInstance::ProcessFootLockBaseChange(const float IkAmount, FHodgeAlsFootState& FootState,
...
void UHodgeAlsAnimationInstance::RefreshFootLock(const float IkAmount, FHodgeAlsFootState& FootState, const FName& LockCurveName,
                                            const FTransform& ComponentTransformInverse, const float DeltaTime) const
{
	auto NewLockAmount{GetCurveValueClamped01(LockCurveName)};

	if (LocomotionState.bMovingSmooth || LocomotionMode != HodgeAlsLocomotionModeTags::Grounded)
	{
	...
	const auto LockAngle{UHodgeAlsVector::AngleBetweenSignedXY(ComponentRelativeThighAxis, FootState.LockComponentRelativeLocation)};
...
void UHodgeAlsAnimationInstance::PlayQuickStopAnimation()
{
	if (!IsValid(Settings))
	{
		return;
	...
	RemainingYawAngle = UHodgeAlsRotation::RemapAngleForCounterClockwiseRotation(RemainingYawAngle);
...
void UHodgeAlsAnimationInstance::PlayTransitionAnimation(UAnimSequenceBase* Sequence, const float BlendInDuration, const float BlendOutDuration,
                                                    const float PlayRate, const float StartTime, const bool bFromStandingIdleOnly)
{
	if (bFromStandingIdleOnly && (LocomotionState.bMoving || Stance != HodgeAlsStanceTags::Standing))
	{
		return;
	}
...
void UHodgeAlsAnimationInstance::PlayTransitionLeftAnimation(const float BlendInDuration, const float BlendOutDuration, const float PlayRate,
                                                        const float StartTime, const bool bFromStandingIdleOnly)
{
	...
	PlayTransitionAnimation(Stance == HodgeAlsStanceTags::Crouching
...
void UHodgeAlsAnimationInstance::PlayTransitionRightAnimation(const float BlendInDuration, const float BlendOutDuration, const float PlayRate,
                                                         const float StartTime, const bool bFromStandingIdleOnly)
{
	if (!IsValid(Settings))
	{
...
void UHodgeAlsAnimationInstance::StopTransitionAndTurnInPlaceAnimations(const float BlendOutDuration)
{
...
void UHodgeAlsAnimationInstance::RefreshTransitions()
{
	...
	TransitionsState.bTransitionsAllowed = FAnimWeight::IsFullWeight(GetCurveValue(UHodgeAlsConstants::AllowTransitionsCurveName()));
}

void UHodgeAlsAnimationInstance::RefreshDynamicTransitions()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshDynamicTransitions"),
	                            STAT_UHodgeAlsAnimationInstance_RefreshDynamicTransitions, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
	...
	{
		DynamicTransitionSequence = Stance == HodgeAlsStanceTags::Crouching
...
void UHodgeAlsAnimationInstance::PlayQueuedTransitionAnimation()
{
	check(IsInGameThread())
	...
	PlaySlotAnimationAsDynamicMontage(TransitionsState.QueuedTransitionSequence, UHodgeAlsConstants::TransitionSlotName(),
...
void UHodgeAlsAnimationInstance::StopQueuedTransitionAndTurnInPlaceAnimations()
{
	check(IsInGameThread())
	...
	StopSlotAnimation(TransitionsState.QueuedStopTransitionsBlendOutDuration, UHodgeAlsConstants::TransitionSlotName());
	StopSlotAnimation(TransitionsState.QueuedStopTransitionsBlendOutDuration, UHodgeAlsConstants::TurnInPlaceStandingSlotName());
	StopSlotAnimation(TransitionsState.QueuedStopTransitionsBlendOutDuration, UHodgeAlsConstants::TurnInPlaceCrouchingSlotName());

	TransitionsState.bStopTransitionsQueued = false;
	TransitionsState.QueuedStopTransitionsBlendOutDuration = 0.0f;
}

bool UHodgeAlsAnimationInstance::IsRotateInPlaceAllowed()
{
	return RotationMode == HodgeAlsRotationModeTags::Aiming || ViewMode == HodgeAlsViewModeTags::FirstPerson;
}

void UHodgeAlsAnimationInstance::RefreshRotateInPlace()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshRotateInPlace"),
	                            STAT_UHodgeAlsAnimationInstance_RefreshRotateInPlace, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
...
bool UHodgeAlsAnimationInstance::IsTurnInPlaceAllowed()
{
	return RotationMode == HodgeAlsRotationModeTags::ViewDirection && ViewMode != HodgeAlsViewModeTags::FirstPerson;
}

void UHodgeAlsAnimationInstance::InitializeTurnInPlace()
{
	TurnInPlaceState.ActivationDelay = 0.0f;
}

void UHodgeAlsAnimationInstance::RefreshTurnInPlace()
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld())
	{
	...
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UHodgeAlsAnimationInstance::RefreshTurnInPlace"),
	                            STAT_UHodgeAlsAnimationInstance_RefreshTurnInPlace, STATGROUP_HodgeAls)
	TRACE_CPUPROFILER_EVENT_SCOPE(__FUNCTION__);
	...
	const auto bTurnLeft{UHodgeAlsRotation::RemapAngleForCounterClockwiseRotation(ViewState.YawAngle) <= 0.0f};

	UHodgeAlsTurnInPlaceSettings* TurnInPlaceSettings{nullptr};
	FName TurnInPlaceSlotName;

	if (Stance == HodgeAlsStanceTags::Standing)
	{
		TurnInPlaceSlotName = UHodgeAlsConstants::TurnInPlaceStandingSlotName();
	...
	else if (Stance == HodgeAlsStanceTags::Crouching)
	{
		TurnInPlaceSlotName = UHodgeAlsConstants::TurnInPlaceCrouchingSlotName();
	...
	if (IsValid(TurnInPlaceSettings) && HODGEALS_ENSURE(IsValid(TurnInPlaceSettings->Sequence)))
	{
		// Animation montages can't be played in the worker thread, so queue them up to play later in the game thread.
...
void UHodgeAlsAnimationInstance::PlayQueuedTurnInPlaceAnimation()
{
	check(IsInGameThread())
...
void UHodgeAlsAnimationInstance::RefreshRagdollingOnGameThread()
{
	check(IsInGameThread())
	...
	RagdollingState.FlailPlayRate = UHodgeAlsMath::Clamp01(UE_REAL_TO_FLOAT(Character->GetRagdollingState().Velocity.Size() / ReferenceSpeed));
}

FPoseSnapshot& UHodgeAlsAnimationInstance::SnapshotFinalRagdollPose()
{
	check(IsInGameThread())
...
float UHodgeAlsAnimationInstance::GetCurveValueClamped01(const FName& CurveName) const
{
	return UHodgeAlsMath::Clamp01(GetCurveValue(CurveName));
}
