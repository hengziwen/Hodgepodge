...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HodgeAlsCharacterMovementComponent)

void FHodgeAlsCharacterNetworkMoveData::ClientFillNetworkMoveData(const FSavedMove_Character& Move, const ENetworkMoveType MoveType)
{
	Super::ClientFillNetworkMoveData(Move, MoveType);

	const auto& SavedMove{static_cast<const FHodgeAlsSavedMove&>(Move)};
...
bool FHodgeAlsCharacterNetworkMoveData::Serialize(UCharacterMovementComponent& Movement, FArchive& Archive,
                                             UPackageMap* Map, const ENetworkMoveType MoveType)
{
	Super::Serialize(Movement, Archive, Map, MoveType);

	NetSerializeOptionalValue(Archive.IsSaving(), Archive, RotationMode, HodgeAlsRotationModeTags::ViewDirection.GetTag(), Map);
	NetSerializeOptionalValue(Archive.IsSaving(), Archive, Stance, HodgeAlsStanceTags::Standing.GetTag(), Map);
	NetSerializeOptionalValue(Archive.IsSaving(), Archive, MaxAllowedGait, HodgeAlsGaitTags::Running.GetTag(), Map);

	return !Archive.IsError();
}

FHodgeAlsCharacterNetworkMoveDataContainer::FHodgeAlsCharacterNetworkMoveDataContainer()
{
...
void FHodgeAlsSavedMove::Clear()
{
	Super::Clear();

	RotationMode = HodgeAlsRotationModeTags::ViewDirection;
	Stance = HodgeAlsStanceTags::Standing;
	MaxAllowedGait = HodgeAlsGaitTags::Running;
}

void FHodgeAlsSavedMove::SetMoveFor(ACharacter* Character, const float NewDeltaTime, const FVector& NewAcceleration,
                               FNetworkPredictionData_Client_Character& PredictionData)
{
	...
	const auto* Movement{Cast<UHodgeAlsCharacterMovementComponent>(Character->GetCharacterMovement())};
	if (IsValid(Movement))
	{
...
bool FHodgeAlsSavedMove::CanCombineWith(const FSavedMovePtr& NewMovePtr, ACharacter* Character, const float MaxDeltaTime) const
{
	const auto* NewMove{static_cast<FHodgeAlsSavedMove*>(NewMovePtr.Get())};
...
void FHodgeAlsSavedMove::CombineWith(const FSavedMove_Character* PreviousMove, ACharacter* Character,
                                APlayerController* Player, const FVector& PreviousStartLocation)
{
	...
	// undesirable because it will erase our rotation changes made in the AHodgeAlsCharacter class. So, to keep the rotation unchanged,
...
void FHodgeAlsSavedMove::PrepMoveFor(ACharacter* Character)
{
	Super::PrepMoveFor(Character);

	auto* Movement{Cast<UHodgeAlsCharacterMovementComponent>(Character->GetCharacterMovement())};
	if (IsValid(Movement))
	{
...
FHodgeAlsNetworkPredictionData::FHodgeAlsNetworkPredictionData(const UCharacterMovementComponent& Movement) : Super{Movement} {}

FSavedMovePtr FHodgeAlsNetworkPredictionData::AllocateNewMove()
{
	return MakeShared<FHodgeAlsSavedMove>();
}

UHodgeAlsCharacterMovementComponent::UHodgeAlsCharacterMovementComponent()
{
...
#if WITH_EDITOR
bool UHodgeAlsCharacterMovementComponent::CanEditChange(const FProperty* Property) const
{
...
void UHodgeAlsCharacterMovementComponent::BeginPlay()
{
	HODGEALS_ENSURE_MESSAGE(!bUseControllerDesiredRotation && !bOrientRotationToMovement,
	                   TEXT("These settings are not allowed and must be turned off!"));

	Super::BeginPlay();
}

FVector UHodgeAlsCharacterMovementComponent::ConsumeInputVector()
{
	...
	if (!bIgnoreBaseRotation && UHodgeAlsUtility::TryGetMovementBaseRotationSpeed(CharacterOwner->GetBasedMovement(), BaseRotationSpeed))
	{
...
void UHodgeAlsCharacterMovementComponent::SetMovementMode(const EMovementMode NewMovementMode, const uint8 NewCustomMode)
{
...
void UHodgeAlsCharacterMovementComponent::OnMovementModeChanged(const EMovementMode PreviousMovementMode, const uint8 PreviousCustomMode)
{
...
bool UHodgeAlsCharacterMovementComponent::ShouldPerformAirControlForPathFollowing() const
{
	return !bInputBlocked && Super::ShouldPerformAirControlForPathFollowing();
}

void UHodgeAlsCharacterMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
	// Ignore the parent implementation of this function and provide our own, because the parent
	// implementation has no effect when we ignore rotation changes in AHodgeAlsCharacter::FaceRotation().
...
bool UHodgeAlsCharacterMovementComponent::ApplyRequestedMove(const float DeltaTime, const float CurrentMaxAcceleration,
...
void UHodgeAlsCharacterMovementComponent::CalcVelocity(const float DeltaTime, const float Friction,
...
float UHodgeAlsCharacterMovementComponent::GetMaxAcceleration() const
{
	if (IsMovingOnGround())
	{
...
void UHodgeAlsCharacterMovementComponent::ControlledCharacterMove(const FVector& InputVector, const float DeltaTime)
{
...
void UHodgeAlsCharacterMovementComponent::PhysicsRotation(const float DeltaTime)
{
...
void UHodgeAlsCharacterMovementComponent::MoveSmooth(const FVector& InVelocity, const float DeltaTime, FStepDownResult* StepDownResult)
{
	if (IsMovingOnGround())
	{
...
void UHodgeAlsCharacterMovementComponent::PhysWalking(const float DeltaTime, int32 IterationsCount)
{
	RefreshGroundedMovementSettings();
		...
		else
		{
			...
			if (CurrentFloor.IsWalkableFloor())
			{
				if (ShouldCatchAir(OldFloor, CurrentFloor))
				{
					...
					if (IsMovingOnGround())
					{
						// TODO Start of custom HODGEALS code block.

						ApplyPendingPenetrationAdjustment();

						// TODO End of custom HODGEALS code block.
...
void UHodgeAlsCharacterMovementComponent::PhysNavWalking(const float DeltaTime, const int32 IterationsCount)
{
	RefreshGroundedMovementSettings();
...
void UHodgeAlsCharacterMovementComponent::PhysCustom(const float DeltaTime, int32 IterationsCount)
{
	if (DeltaTime < MIN_TICK_TIME)
	{
...
void UHodgeAlsCharacterMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance,
...
void UHodgeAlsCharacterMovementComponent::PerformMovement(const float DeltaTime)
{
...
FNetworkPredictionData_Client* UHodgeAlsCharacterMovementComponent::GetPredictionData_Client() const
{
	if (ClientPredictionData == nullptr)
	{
		auto* MutableThis{const_cast<ThisClass*>(this)};

		MutableThis->ClientPredictionData = new FHodgeAlsNetworkPredictionData{*this};
	}

	return ClientPredictionData;
}

void UHodgeAlsCharacterMovementComponent::SmoothClientPosition(const float DeltaTime)
{
...
void UHodgeAlsCharacterMovementComponent::MoveAutonomous(const float ClientTimeStamp, const float DeltaTime,
                                                    const uint8 CompressedFlags, const FVector& NewAcceleration)
{
	const auto* MoveData{static_cast<FHodgeAlsCharacterNetworkMoveData*>(GetCurrentNetworkMoveData())};
	if (MoveData != nullptr)
	{
	...
	if (IsValid(Controller) && IsNetMode(NM_ListenServer) && CharacterOwner->GetRemoteRole() == ROLE_AutonomousProxy)
	{
		...
		auto* Character{Cast<AHodgeAlsCharacter>(CharacterOwner)};
...
void UHodgeAlsCharacterMovementComponent::SavePenetrationAdjustment(const FHitResult& Hit)
{
...
void UHodgeAlsCharacterMovementComponent::ApplyPendingPenetrationAdjustment()
{
...
void UHodgeAlsCharacterMovementComponent::SetMovementSettings(UHodgeAlsMovementSettings* NewMovementSettings)
{
	HODGEALS_ENSURE(IsValid(NewMovementSettings));
...
void UHodgeAlsCharacterMovementComponent::RefreshGaitSettings()
{
	if (!HODGEALS_ENSURE(IsValid(MovementSettings)))
	{
		return;
	}

	const auto* StanceSettings{MovementSettings->RotationModes.Find(RotationMode)};
	const auto* NewGaitSettings{HODGEALS_ENSURE(StanceSettings != nullptr) ? StanceSettings->Stances.Find(Stance) : nullptr};

	GaitSettings = HODGEALS_ENSURE(NewGaitSettings != nullptr) ? *NewGaitSettings : FHodgeAlsMovementGaitSettings{};
}

void UHodgeAlsCharacterMovementComponent::SetRotationMode(const FGameplayTag& NewRotationMode)
{
...
void UHodgeAlsCharacterMovementComponent::SetStance(const FGameplayTag& NewStance)
{
	if (Stance != NewStance)
	{
		Stance = NewStance;
...
void UHodgeAlsCharacterMovementComponent::RefreshGroundedMovementSettings()
{
	    ...
	    IsValid(MovementSettings))
	{
		...
		// Ideally we should use actor rotation here instead of view rotation, but we can't do that because HODGEALS has
		// full control over actor rotation and it is not synchronized over the network, so it would cause jitter.

		const auto RelativeViewRotation{UHodgeAlsRotation::GetTwist(ViewRotation.Quaternion(), -GetGravityDirection())};

		const FVector2D RelativeVelocity{RelativeViewRotation.UnrotateVector(Velocity)};
		const auto VelocityAngle{UHodgeAlsVector::DirectionToAngle(RelativeVelocity)};
	...
	if (MaxAllowedGait == HodgeAlsGaitTags::Walking)
	{
		MaxWalkSpeed = WalkSpeed;
	}
	else if (MaxAllowedGait == HodgeAlsGaitTags::Running)
	{
		MaxWalkSpeed = RunSpeed;
	}
	else if (MaxAllowedGait == HodgeAlsGaitTags::Sprinting)
	{
	...
	if (HODGEALS_ENSURE(IsValid(GaitSettings.AccelerationAndDecelerationAndGroundFrictionCurve)))
	{
...
void UHodgeAlsCharacterMovementComponent::SetMovementModeLocked(const bool bNewMovementModeLocked)
{
	bMovementModeLocked = bNewMovementModeLocked;
}

void UHodgeAlsCharacterMovementComponent::SetInputBlocked(const bool bNewInputBlocked)
{
	bInputBlocked = bNewInputBlocked;
}

bool UHodgeAlsCharacterMovementComponent::TryConsumePrePenetrationAdjustmentVelocity(FVector& OutVelocity)
{
