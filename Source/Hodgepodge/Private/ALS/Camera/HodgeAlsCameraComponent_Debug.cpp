...
#define LOCTEXT_NAMESPACE "HodgeAlsCameraComponentDebug"

void UHodgeAlsCameraComponent::DisplayDebug(const UCanvas* Canvas, const FDebugDisplayInfo& DisplayInfo, float& VerticalLocation) const
{
	...
	if (!DisplayInfo.IsDisplayOn(UHodgeAlsCameraConstants::CameraCurvesDebugDisplayName()) &&
	    !DisplayInfo.IsDisplayOn(UHodgeAlsCameraConstants::CameraShapesDebugDisplayName()) &&
	    !DisplayInfo.IsDisplayOn(UHodgeAlsCameraConstants::CameraTracesDebugDisplayName()))
	{
	...
	static const auto CameraCurvesHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.CameraCurves (Shift + 6)")})};
	...
	static const auto ShapesHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.CameraShapes (Shift + 7)")})};

	if (DisplayInfo.IsDisplayOn(UHodgeAlsCameraConstants::CameraShapesDebugDisplayName()))
	{
	...
	static const auto TracesHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.CameraTraces (Shift + 8)")})};

	if (DisplayInfo.IsDisplayOn(UHodgeAlsCameraConstants::CameraTracesDebugDisplayName()))
	{
...
void UHodgeAlsCameraComponent::DisplayDebugHeader(const UCanvas* Canvas, const FText& HeaderText, const FLinearColor& HeaderColor,
...
void UHodgeAlsCameraComponent::DisplayDebugCurves(const UCanvas* Canvas, const float Scale,
                                             const float HorizontalLocation, float& VerticalLocation) const
{
	...
	for (const auto& CurveName : CurveNames)
	{
		...
		Text.SetColor(FMath::Lerp(FLinearColor::Gray, FLinearColor::White, UHodgeAlsMath::Clamp01(FMath::Abs(CurveValue))));
...
void UHodgeAlsCameraComponent::DisplayDebugShapes(const UCanvas* Canvas, const float Scale,
                                             const float HorizontalLocation, float& VerticalLocation) const
{
	VerticalLocation += 4.0f * Scale;
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(FString{UHodgeAlsUtility::BoolToString(bRightShoulder)}, false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
...
void UHodgeAlsCameraComponent::DisplayDebugTraces(const UCanvas* Canvas, const float Scale,
                                             const float HorizontalLocation, float& VerticalLocation) const
{
	VerticalLocation += 4.0f * Scale;
