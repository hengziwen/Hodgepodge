#include "ALS/Nodes/HodgeAlsRigUnit_ChainLength.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HodgeAlsRigUnit_ChainLength)

namespace HodgeAlsChainLengthRigUnit
{
...
FHodgeAlsRigUnit_ChainLength_Execute()
{
	...
	Length = HodgeAlsChainLengthRigUnit::CalculateChainLength(AncestorTransformElement, DescendantTransformElement,
	                                                     Hierarchy, TransformType, VisitedElements);

	if (Length < 0.0f)
	{
		Length = FMath::Max(0.0f, HodgeAlsChainLengthRigUnit::CalculateChainLength(DescendantTransformElement, AncestorTransformElement,
		                                                                      Hierarchy, TransformType, VisitedElements));
	}
}
