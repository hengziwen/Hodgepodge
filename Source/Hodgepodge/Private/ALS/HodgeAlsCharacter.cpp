...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HodgeAlsCharacter)

namespace HodgeAlsCharacterConstants
{
	constexpr auto MinAimingYawAngleLimit{70.0f};
}

AHodgeAlsCharacter::AHodgeAlsCharacter(const FObjectInitializer& ObjectInitializer) : Super{
	ObjectInitializer.SetDefaultSubobjectClass<UHodgeAlsCharacterMovementComponent>(CharacterMovementComponentName)
}
{
	...
	HodgeAlsCharacterMovement = Cast<UHodgeAlsCharacterMovementComponent>(GetCharacterMovement());
...
#if WITH_EDITOR
bool AHodgeAlsCharacter::CanEditChange(const FProperty* Property) const
{
...
void AHodgeAlsCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
...
void AHodgeAlsCharacter::PreRegisterAllComponents()
{
	...
	RotationMode = bDesiredAiming ? HodgeAlsRotationModeTags::Aiming : DesiredRotationMode;
...
void AHodgeAlsCharacter::PostRegisterAllComponents()
{
...
void AHodgeAlsCharacter::PostInitializeComponents()
{
	...
	HodgeAlsCharacterMovement->OnPhysicsRotation.AddUObject(this, &ThisClass::CharacterMovement_OnPhysicsRotation);

	// Pass current movement settings to the movement component.

	HodgeAlsCharacterMovement->SetMovementSettings(MovementSettings);

	AnimationInstance = Cast<UHodgeAlsAnimationInstance>(GetMesh()->GetAnimInstance());

	Super::PostInitializeComponents();
}

void AHodgeAlsCharacter::BeginPlay()
{
	HODGEALS_ENSURE(IsValid(Settings));
	HODGEALS_ENSURE(IsValid(MovementSettings));
	HODGEALS_ENSURE(AnimationInstance.IsValid());

	HODGEALS_ENSURE_MESSAGE(!bUseControllerRotationPitch && !bUseControllerRotationYaw && !bUseControllerRotationRoll,
	...
	if (GetLocalRole() >= ROLE_AutonomousProxy)
	{
		...
		// AHodgeAlsCharacter::PostNetReceiveLocationAndRotation() and AHodgeAlsCharacter::OnRep_ReplicatedBasedMovement().
	...
	HodgeAlsCharacterMovement->SetStance(Stance);

	RefreshGait();

	RefreshRotationMode();

	HodgeAlsCharacterMovement->SetRotationMode(RotationMode);

	OnOverlayModeChanged(OverlayMode);
}

void AHodgeAlsCharacter::CalcCamera(const float DeltaTime, FMinimalViewInfo& ViewInfo)
{
...
void AHodgeAlsCharacter::PostNetReceiveLocationAndRotation()
{
	...
	// Ignore server-replicated rotation on simulated proxies because HODGEALS itself has full control over character rotation.
...
void AHodgeAlsCharacter::OnRep_ReplicatedBasedMovement()
{
...
void AHodgeAlsCharacter::Tick(const float DeltaTime)
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("AHodgeAlsCharacter::Tick"), STAT_AHodgeAlsCharacter_Tick, STATGROUP_HodgeAls)
...
void AHodgeAlsCharacter::PossessedBy(AController* NewController)
{
...
void AHodgeAlsCharacter::Restart()
{
	Super::Restart();
...
bool AHodgeAlsCharacter::OnCalculateCamera_Implementation(float DeltaTime, FMinimalViewInfo& ViewInfo)
{
	return false;
}

void AHodgeAlsCharacter::RefreshMeshProperties() const
{
...
void AHodgeAlsCharacter::RefreshMovementBase()
{
...
void AHodgeAlsCharacter::SetViewMode(const FGameplayTag& NewViewMode)
{
	SetViewMode(NewViewMode, true);
}

void AHodgeAlsCharacter::SetViewMode(const FGameplayTag& NewViewMode, const bool bSendRpc)
{
...
void AHodgeAlsCharacter::ClientSetViewMode_Implementation(const FGameplayTag& NewViewMode)
{
	SetViewMode(NewViewMode, false);
}

void AHodgeAlsCharacter::ServerSetViewMode_Implementation(const FGameplayTag& NewViewMode)
{
	SetViewMode(NewViewMode, false);
}

void AHodgeAlsCharacter::OnMovementModeChanged(const EMovementMode PreviousMovementMode, const uint8 PreviousCustomMode)
{
	...
	switch (GetCharacterMovement()->MovementMode)
	{
		...
		case MOVE_NavWalking:
			SetLocomotionMode(HodgeAlsLocomotionModeTags::Grounded);
			break;

		case MOVE_Falling:
			SetLocomotionMode(HodgeAlsLocomotionModeTags::InAir);
			break;
...
void AHodgeAlsCharacter::SetLocomotionMode(const FGameplayTag& NewLocomotionMode)
{
...
void AHodgeAlsCharacter::NotifyLocomotionModeChanged(const FGameplayTag& PreviousLocomotionMode)
{
	ApplyDesiredStance();

	if (LocomotionMode == HodgeAlsLocomotionModeTags::Grounded &&
	    PreviousLocomotionMode == HodgeAlsLocomotionModeTags::InAir)
	{
	...
	else if (LocomotionMode == HodgeAlsLocomotionModeTags::InAir &&
...
void AHodgeAlsCharacter::SetDesiredAiming(const bool bNewDesiredAiming)
{
	SetDesiredAiming(bNewDesiredAiming, true);
}

void AHodgeAlsCharacter::SetDesiredAiming(const bool bNewDesiredAiming, const bool bSendRpc)
{
...
void AHodgeAlsCharacter::ClientSetDesiredAiming_Implementation(const bool bNewDesiredAiming)
{
	SetDesiredAiming(bNewDesiredAiming, false);
}

void AHodgeAlsCharacter::ServerSetDesiredAiming_Implementation(const bool bNewDesiredAiming)
{
	SetDesiredAiming(bNewDesiredAiming, false);
}

void AHodgeAlsCharacter::OnReplicated_DesiredAiming(const bool bPreviousDesiredAiming)
{
	OnDesiredAimingChanged(bPreviousDesiredAiming);
}

void AHodgeAlsCharacter::OnDesiredAimingChanged_Implementation(const bool bPreviousDesiredAiming) {}

void AHodgeAlsCharacter::SetDesiredRotationMode(const FGameplayTag& NewDesiredRotationMode)
{
	SetDesiredRotationMode(NewDesiredRotationMode, true);
}

void AHodgeAlsCharacter::SetDesiredRotationMode(const FGameplayTag& NewDesiredRotationMode, const bool bSendRpc)
{
...
void AHodgeAlsCharacter::ClientSetDesiredRotationMode_Implementation(const FGameplayTag& NewDesiredRotationMode)
{
	SetDesiredRotationMode(NewDesiredRotationMode, false);
}

void AHodgeAlsCharacter::ServerSetDesiredRotationMode_Implementation(const FGameplayTag& NewDesiredRotationMode)
{
	SetDesiredRotationMode(NewDesiredRotationMode, false);
}

void AHodgeAlsCharacter::SetRotationMode(const FGameplayTag& NewRotationMode)
{
	HodgeAlsCharacterMovement->SetRotationMode(NewRotationMode);
...
void AHodgeAlsCharacter::NotifyRotationModeChanged(const FGameplayTag& PreviousRotationMode)
{
...
void AHodgeAlsCharacter::RefreshRotationMode()
{
	const auto bAiming{bDesiredAiming || DesiredRotationMode == HodgeAlsRotationModeTags::Aiming};
	const auto bSprinting{HodgeAlsCharacterMovement->GetMaxAllowedGait() == HodgeAlsGaitTags::Sprinting};

	if (ViewMode == HodgeAlsViewModeTags::FirstPerson)
	{
		if (LocomotionMode == HodgeAlsLocomotionModeTags::InAir)
		{
			if (bAiming && Settings->bAllowAimingWhenInAir)
			{
				SetRotationMode(HodgeAlsRotationModeTags::Aiming);
			}
			else
			{
				SetRotationMode(HodgeAlsRotationModeTags::ViewDirection);
			}
	...
	if (bSprinting)
	{
		...
		{
			SetRotationMode(HodgeAlsRotationModeTags::VelocityDirection);
		}
		else
		{
			SetRotationMode(DesiredRotationMode);
...
void AHodgeAlsCharacter::SetDesiredStance(const FGameplayTag& NewDesiredStance)
{
	SetDesiredStance(NewDesiredStance, true);
}

void AHodgeAlsCharacter::SetDesiredStance(const FGameplayTag& NewDesiredStance, const bool bSendRpc)
{
...
void AHodgeAlsCharacter::ClientSetDesiredStance_Implementation(const FGameplayTag& NewDesiredStance)
{
	SetDesiredStance(NewDesiredStance, false);
}

void AHodgeAlsCharacter::ServerSetDesiredStance_Implementation(const FGameplayTag& NewDesiredStance)
{
	SetDesiredStance(NewDesiredStance, false);
}

void AHodgeAlsCharacter::ApplyDesiredStance()
{
	if (!LocomotionAction.IsValid())
	{
		if (LocomotionMode == HodgeAlsLocomotionModeTags::Grounded)
		{
			if (DesiredStance == HodgeAlsStanceTags::Standing)
			{
				UnCrouch();
			}
			else if (DesiredStance == HodgeAlsStanceTags::Crouching)
			{
				Crouch();
			}
		}
	...
	else if (LocomotionAction == HodgeAlsLocomotionActionTags::Rolling && Settings->Rolling.bCrouchOnStart)
	{
		Crouch();
	}
}

bool AHodgeAlsCharacter::CanCrouch() const
{
...
void AHodgeAlsCharacter::OnStartCrouch(const float HalfHeightAdjust, const float ScaledHalfHeightAdjust)
{
	...
	SetStance(HodgeAlsStanceTags::Crouching);
}

void AHodgeAlsCharacter::OnEndCrouch(const float HalfHeightAdjust, const float ScaledHalfHeightAdjust)
{
	auto* PredictionData{GetCharacterMovement()->GetPredictionData_Client_Character()};
	...
	if (PredictionData != nullptr && GetLocalRole() <= ROLE_SimulatedProxy &&
	    ScaledHalfHeightAdjust > 0.0f && IsPlayingNetworkedRootMotionMontage())
	{
		// Same fix as in AHodgeAlsCharacter::OnStartCrouch().
	...
	SetStance(HodgeAlsStanceTags::Standing);
}

void AHodgeAlsCharacter::SetStance(const FGameplayTag& NewStance)
{
	HodgeAlsCharacterMovement->SetStance(NewStance);
...
void AHodgeAlsCharacter::OnStanceChanged_Implementation(const FGameplayTag& PreviousStance) {}

void AHodgeAlsCharacter::SetDesiredGait(const FGameplayTag& NewDesiredGait)
{
	SetDesiredGait(NewDesiredGait, true);
}

void AHodgeAlsCharacter::SetDesiredGait(const FGameplayTag& NewDesiredGait, const bool bSendRpc)
{
...
void AHodgeAlsCharacter::ClientSetDesiredGait_Implementation(const FGameplayTag& NewDesiredGait)
{
	SetDesiredGait(NewDesiredGait, false);
}

void AHodgeAlsCharacter::ServerSetDesiredGait_Implementation(const FGameplayTag& NewDesiredGait)
{
	SetDesiredGait(NewDesiredGait, false);
}

void AHodgeAlsCharacter::SetGait(const FGameplayTag& NewGait)
{
	if (Gait != NewGait)
	{
...
void AHodgeAlsCharacter::OnGaitChanged_Implementation(const FGameplayTag& PreviousGait) {}

void AHodgeAlsCharacter::RefreshGait()
{
	if (LocomotionMode != HodgeAlsLocomotionModeTags::Grounded)
	{
		return;
	...
	HodgeAlsCharacterMovement->SetMaxAllowedGait(MaxAllowedGait);
...
FGameplayTag AHodgeAlsCharacter::CalculateMaxAllowedGait() const
{
	...
	if (DesiredGait != HodgeAlsGaitTags::Sprinting)
	{
		return DesiredGait;
	}
	...
	if (CanSprint())
	{
		return HodgeAlsGaitTags::Sprinting;
	}

	return HodgeAlsGaitTags::Running;
}

FGameplayTag AHodgeAlsCharacter::CalculateActualGait(const FGameplayTag& MaxAllowedGait) const
{
	...
	const auto& GaitSettings{HodgeAlsCharacterMovement->GetGaitSettings()};

	if (LocomotionState.Speed < GaitSettings.GetMaxWalkSpeed() + 10.0f)
	{
		return HodgeAlsGaitTags::Walking;
	}

	if (LocomotionState.Speed < GaitSettings.GetMaxRunSpeed() + 10.0f || MaxAllowedGait != HodgeAlsGaitTags::Sprinting)
	{
		return HodgeAlsGaitTags::Running;
	}
...
bool AHodgeAlsCharacter::CanSprint() const
{
	...
	if (!LocomotionState.bHasInput || Stance != HodgeAlsStanceTags::Standing ||
	    ((bDesiredAiming || DesiredRotationMode == HodgeAlsRotationModeTags::Aiming) && !Settings->bSprintHasPriorityOverAiming))
	{
		return false;
	}

	if (ViewMode != HodgeAlsViewModeTags::FirstPerson &&
	    (DesiredRotationMode == HodgeAlsRotationModeTags::VelocityDirection || Settings->bRotateToVelocityWhenSprinting))
	{
		return true;
	}
...
void AHodgeAlsCharacter::SetOverlayMode(const FGameplayTag& NewOverlayMode)
{
	SetOverlayMode(NewOverlayMode, true);
}

void AHodgeAlsCharacter::SetOverlayMode(const FGameplayTag& NewOverlayMode, const bool bSendRpc)
{
...
void AHodgeAlsCharacter::ClientSetOverlayMode_Implementation(const FGameplayTag& NewOverlayMode)
{
	SetOverlayMode(NewOverlayMode, false);
}

void AHodgeAlsCharacter::ServerSetOverlayMode_Implementation(const FGameplayTag& NewOverlayMode)
{
	SetOverlayMode(NewOverlayMode, false);
}

void AHodgeAlsCharacter::OnReplicated_OverlayMode(const FGameplayTag& PreviousOverlayMode)
{
	OnOverlayModeChanged(PreviousOverlayMode);
}

void AHodgeAlsCharacter::OnOverlayModeChanged_Implementation(const FGameplayTag& PreviousOverlayMode) {}

void AHodgeAlsCharacter::SetLocomotionAction(const FGameplayTag& NewLocomotionAction)
{
...
void AHodgeAlsCharacter::NotifyLocomotionActionChanged(const FGameplayTag& PreviousLocomotionAction)
{
	if (!LocomotionAction.IsValid())
	{
		HodgeAlsCharacterMovement->SetInputBlocked(false);
	}
...
FRotator AHodgeAlsCharacter::GetViewRotation() const
{
	return ViewState.Rotation;
}

void AHodgeAlsCharacter::SetInputDirection(FVector NewInputDirection)
{
...
void AHodgeAlsCharacter::RefreshInput(const float DeltaTime)
{
	if (GetLocalRole() >= ROLE_AutonomousProxy)
	{
	...
	{
		LocomotionState.InputYawAngle = UE_REAL_TO_FLOAT(UHodgeAlsVector::DirectionToAngleXY(InputDirection));
	}
}

void AHodgeAlsCharacter::SetReplicatedViewRotation(const FRotator& NewViewRotation, const bool bSendRpc)
{
...
void AHodgeAlsCharacter::ServerSetReplicatedViewRotation_Implementation(const FRotator& NewViewRotation)
{
	SetReplicatedViewRotation(NewViewRotation, false);
}

void AHodgeAlsCharacter::OnReplicated_ReplicatedViewRotation()
{
	CorrectViewNetworkSmoothing(ReplicatedViewRotation, MovementBase.bHasRelativeRotation);
}

void AHodgeAlsCharacter::CorrectViewNetworkSmoothing(const FRotator& NewTargetRotation, const bool bRotationIsBaseRelative)
{
...
void AHodgeAlsCharacter::RefreshView(const float DeltaTime)
{
...
void AHodgeAlsCharacter::RefreshViewNetworkSmoothing(const float DeltaTime)
{
	...
	const auto InterpolationAmount{
		UHodgeAlsMath::Clamp01(1.0f - (NetworkSmoothing.ServerTime - NetworkSmoothing.ClientTime) / NetworkSmoothing.Duration)
	};

	if (!FAnimWeight::IsFullWeight(InterpolationAmount))
	{
		NetworkSmoothing.CurrentRotation = UHodgeAlsRotation::LerpRotation(NetworkSmoothing.InitialRotation, NetworkSmoothing.TargetRotation,
		                                                              InterpolationAmount);
	}
	else
	{
...
void AHodgeAlsCharacter::SetDesiredVelocityYawAngle(const float NewVelocityYawAngle)
{
	COMPARE_ASSIGN_AND_MARK_PROPERTY_DIRTY(ThisClass, DesiredVelocityYawAngle, NewVelocityYawAngle, this);
}

void AHodgeAlsCharacter::RefreshLocomotionEarly()
{
	if (!LocomotionState.bMoving &&
	    RotationMode == HodgeAlsRotationModeTags::VelocityDirection &&
...
void AHodgeAlsCharacter::RefreshLocomotion()
{
	...
	{
		LocomotionState.VelocityYawAngle = UE_REAL_TO_FLOAT(UHodgeAlsVector::DirectionToAngleXY(LocomotionState.Velocity));
	}
	...
	if (GetLocalRole() >= ROLE_AutonomousProxy)
	{
		...
		if (Settings->bRotateTowardsDesiredVelocityInVelocityDirectionRotationMode)
		{
			...
			if (HodgeAlsCharacterMovement->TryConsumePrePenetrationAdjustmentVelocity(DesiredVelocity) &&
			    DesiredVelocity.Size2D() >= HasSpeedThreshold)
			{
				bSendInitialVelocityYawAngle = !bHasDesiredVelocity;
				bHasDesiredVelocity = true;

				SetDesiredVelocityYawAngle(UE_REAL_TO_FLOAT(UHodgeAlsVector::DirectionToAngleXY(DesiredVelocity)));
			}
			else
			{
...
void AHodgeAlsCharacter::RefreshLocomotionLate()
{
...
void AHodgeAlsCharacter::ServerSetInitialVelocityYawAngle_Implementation(const float NewVelocityYawAngle)
{
	MulticastSetInitialVelocityYawAngle(NewVelocityYawAngle);
}

void AHodgeAlsCharacter::MulticastSetInitialVelocityYawAngle_Implementation(const float NewVelocityYawAngle)
{
	if (GetLocalRole() != ROLE_AutonomousProxy)
	{
...
void AHodgeAlsCharacter::Jump()
{
	if (Stance == HodgeAlsStanceTags::Standing && !LocomotionAction.IsValid() &&
	    LocomotionMode == HodgeAlsLocomotionModeTags::Grounded)
	{
		Super::Jump();
	}
}

void AHodgeAlsCharacter::OnJumped_Implementation()
{
...
void AHodgeAlsCharacter::MulticastOnJumpedNetworked_Implementation()
{
	if (GetLocalRole() != ROLE_AutonomousProxy)
	{
		OnJumpedNetworked();
...
void AHodgeAlsCharacter::OnJumpedNetworked()
{
...
void AHodgeAlsCharacter::FaceRotation(const FRotator Rotation, const float DeltaTime)
{
	// Left empty intentionally. We ignore rotation changes from external sources because HODGEALS itself has full control over actor rotation.
}

void AHodgeAlsCharacter::CharacterMovement_OnPhysicsRotation(const float DeltaTime)
{
	RefreshRollingPhysics(DeltaTime);
}

void AHodgeAlsCharacter::RefreshGroundedRotation(const float DeltaTime)
{
	if (LocomotionAction.IsValid() || LocomotionMode != HodgeAlsLocomotionModeTags::Grounded)
	{
		return;
	}
	...
	if (!LocomotionState.bMoving)
	{
		...
		if (RotationMode == HodgeAlsRotationModeTags::VelocityDirection)
		{
			...
			else
			{
				...
				// base handled in the AHodgeAlsCharacter::RefreshLocomotionEarly() function.
		...
		if (RotationMode == HodgeAlsRotationModeTags::ViewDirection)
		{
		...
		if (RotationMode == HodgeAlsRotationModeTags::Aiming || ViewMode == HodgeAlsViewModeTags::FirstPerson)
		{
	    ...
	    (LocomotionState.bHasInput || !LocomotionState.bRotationTowardsLastInputDirectionBlocked))
	{
		...
		else
		{
			TargetYawAngle = UE_REAL_TO_FLOAT(
				ViewState.Rotation.Yaw + GetMesh()->GetAnimInstance()->GetCurveValue(UHodgeAlsConstants::RotationYawOffsetCurveName()));
		}
	...
	if (RotationMode == HodgeAlsRotationModeTags::Aiming)
	{
		RefreshGroundedAimingRotation(DeltaTime);
		return;
	}
...
bool AHodgeAlsCharacter::RefreshCustomGroundedMovingRotation(const float DeltaTime)
{
	return false;
}

bool AHodgeAlsCharacter::RefreshCustomGroundedNotMovingRotation(const float DeltaTime)
{
	return false;
}

void AHodgeAlsCharacter::RefreshGroundedAimingRotation(const float DeltaTime)
{
	...
	else
	{
		...
		NewActorRotation.Yaw = UHodgeAlsRotation::ExponentialDecayAngle(UE_REAL_TO_FLOAT(FMath::UnwindDegrees(NewActorRotation.Yaw)),
...
bool AHodgeAlsCharacter::ConstrainAimingRotation(FRotator& ActorRotation, const float DeltaTime, const bool bApplySecondaryConstraint)
{
	...
	if (FMath::Abs(ViewRelativeAngle) <= HodgeAlsCharacterConstants::MinAimingYawAngleLimit + UE_KINDA_SMALL_NUMBER)
	{
		LocomotionState.AimingYawAngleLimit = HodgeAlsCharacterConstants::MinAimingYawAngleLimit;
		return false;
	}

	ViewRelativeAngle = UHodgeAlsRotation::RemapAngleForCounterClockwiseRotation(ViewRelativeAngle);
	...
	if (bApplySecondaryConstraint)
	{
		...
		const auto TargetViewRelativeAngle{
			FMath::Clamp(ViewRelativeAngle, -HodgeAlsCharacterConstants::MinAimingYawAngleLimit, HodgeAlsCharacterConstants::MinAimingYawAngleLimit)
		};

		const auto DeltaAngle{FMath::UnwindDegrees(TargetViewRelativeAngle - ViewRelativeAngle)};
		const auto InterpolationAmount{UHodgeAlsMath::ExponentialDecay(DeltaTime, RotationInterpolationSpeed)};
	...
	{
		LocomotionState.AimingYawAngleLimit = FMath::Max(FMath::Abs(ViewRelativeAngle), HodgeAlsCharacterConstants::MinAimingYawAngleLimit);
	}
...
float AHodgeAlsCharacter::CalculateGroundedMovingRotationInterpolationSpeed() const
{
	...
	const auto* InterpolationSpeedCurve{HodgeAlsCharacterMovement->GetGaitSettings().RotationInterpolationSpeedCurve.Get()};

	static constexpr auto DefaultInterpolationSpeed{5.0f};

	const auto InterpolationSpeed{
		HODGEALS_ENSURE(IsValid(InterpolationSpeedCurve))
			? InterpolationSpeedCurve->GetFloatValue(FMath::Max(1.0f, HodgeAlsCharacterMovement->GetGaitAmount()))
	...
	return InterpolationSpeed * UHodgeAlsMath::LerpClamped(1.0f, MaxInterpolationSpeedMultiplier,
	                                                  ViewState.YawSpeed / ReferenceViewYawSpeed);
}

void AHodgeAlsCharacter::ApplyRotationYawSpeedAnimationCurve(const float DeltaTime)
{
	const auto DeltaYawAngle{GetMesh()->GetAnimInstance()->GetCurveValue(UHodgeAlsConstants::RotationYawSpeedCurveName()) * DeltaTime};
...
void AHodgeAlsCharacter::RefreshInAirRotation(const float DeltaTime)
{
	if (LocomotionAction.IsValid() || LocomotionMode != HodgeAlsLocomotionModeTags::InAir)
	{
		return;
	}
	...
	if (RotationMode == HodgeAlsRotationModeTags::VelocityDirection || RotationMode == HodgeAlsRotationModeTags::ViewDirection)
	{
		switch (Settings->InAirRotationMode)
		{
			case EHodgeAlsInAirRotationMode::RotateToVelocityOnJump:
				if (LocomotionState.bMoving)
				{
			...
			case EHodgeAlsInAirRotationMode::KeepRelativeRotation:
	...
	else if (RotationMode == HodgeAlsRotationModeTags::Aiming)
	{
...
bool AHodgeAlsCharacter::RefreshCustomInAirRotation(const float DeltaTime)
{
	return false;
}

void AHodgeAlsCharacter::RefreshInAirAimingRotation(const float DeltaTime)
{
	...
	auto NewRotation{GetActorRotation()};
	NewRotation.Yaw = UHodgeAlsRotation::ExponentialDecayAngle(UE_REAL_TO_FLOAT(FMath::UnwindDegrees(NewRotation.Yaw)),
...
void AHodgeAlsCharacter::SetRotationSmooth(const float TargetYawAngle, const float DeltaTime, const float InterpolationSpeed)
{
...
void AHodgeAlsCharacter::SetRotationExtraSmooth(const float TargetYawAngle, const float DeltaTime,
...
void AHodgeAlsCharacter::SetRotationInstant(const float TargetYawAngle, const ETeleportType Teleport)
{
	SetTargetYawAngle(TargetYawAngle);
...
void AHodgeAlsCharacter::RefreshTargetYawAngleUsingActorRotation()
{
	const auto YawAngle{UE_REAL_TO_FLOAT(GetActorRotation().Yaw)};
...
void AHodgeAlsCharacter::SetTargetYawAngle(const float TargetYawAngle)
{
...
void AHodgeAlsCharacter::SetTargetYawAngleSmooth(const float TargetYawAngle, const float DeltaTime, const float RotationSpeed)
{
	LocomotionState.TargetYawAngle = FMath::UnwindDegrees(TargetYawAngle);

	LocomotionState.SmoothTargetYawAngle = UHodgeAlsRotation::InterpolateAngleConstant(
		LocomotionState.SmoothTargetYawAngle, LocomotionState.TargetYawAngle, DeltaTime, RotationSpeed);

	RefreshViewRelativeTargetYawAngle();
}

void AHodgeAlsCharacter::RefreshViewRelativeTargetYawAngle()
{
	LocomotionState.ViewRelativeTargetYawAngle = FMath::UnwindDegrees(UE_REAL_TO_FLOAT(
