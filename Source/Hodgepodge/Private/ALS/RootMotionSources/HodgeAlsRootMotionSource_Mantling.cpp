...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HodgeAlsRootMotionSource_Mantling)

FHodgeAlsRootMotionSource_Mantling::FHodgeAlsRootMotionSource_Mantling()
{
	Priority = 1000;
}

FRootMotionSource* FHodgeAlsRootMotionSource_Mantling::Clone() const
{
	return new FHodgeAlsRootMotionSource_Mantling{*this};
}

bool FHodgeAlsRootMotionSource_Mantling::Matches(const FRootMotionSource* Other) const
{
	...
	const auto* OtherCasted{static_cast<const FHodgeAlsRootMotionSource_Mantling*>(Other)};

	return MantlingSettings == OtherCasted->MantlingSettings &&
	       TargetPrimitive == OtherCasted->TargetPrimitive;
}

void FHodgeAlsRootMotionSource_Mantling::PrepareRootMotion(const float SimulationDeltaTime, const float DeltaTime,
                                                      const ACharacter& Character, const UCharacterMovementComponent& Movement)
{
	SetTime(GetTime() + SimulationDeltaTime);

	if (!HODGEALS_ENSURE(GetDuration() > UE_SMALL_NUMBER) || DeltaTime <= UE_SMALL_NUMBER || !TargetPrimitive.IsValid())
	{
	...
	const auto Twist{UHodgeAlsRotation::GetTwist(TargetTransform.GetRotation(), -Character.GetGravityDirection())};
	...
	const auto CurrentAnimationLocation{UHodgeAlsMontageUtility::ExtractRootTransformFromMontage(Montage, MontageTime).GetLocation()};
...
bool FHodgeAlsRootMotionSource_Mantling::NetSerialize(FArchive& Archive, UPackageMap* Map, bool& bSuccess)
{
...
UScriptStruct* FHodgeAlsRootMotionSource_Mantling::GetScriptStruct() const
{
	return StaticStruct();
}

FString FHodgeAlsRootMotionSource_Mantling::ToSimpleString() const
{
	TStringBuilder<256> StringBuilder{
		InPlace, HODGEALS_GET_TYPE_STRING(FHodgeAlsRootMotionSource_Mantling), TEXTVIEW(" ("), InstanceName, TEXTVIEW(", "), LocalID, TEXT(')')
	};

	return FString{StringBuilder};
}

void FHodgeAlsRootMotionSource_Mantling::AddReferencedObjects(FReferenceCollector& Collector)
{
