...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HodgeAlsMath)

float UHodgeAlsMath::SpringDampFloat(FHodgeAlsSpringFloatState& SpringState, const float Current, const float Target, const float DeltaTime,
...
EHodgeAlsMovementDirection UHodgeAlsMath::CalculateMovementDirection(const float Angle, const float ForwardHalfAngle, const float AngleThreshold)
{
	if (Angle >= -ForwardHalfAngle - AngleThreshold && Angle <= ForwardHalfAngle + AngleThreshold)
	{
		return EHodgeAlsMovementDirection::Forward;
	}

	if (Angle >= ForwardHalfAngle - AngleThreshold && Angle <= 180.0f - ForwardHalfAngle + AngleThreshold)
	{
		return EHodgeAlsMovementDirection::Right;
	}

	if (Angle <= -(ForwardHalfAngle - AngleThreshold) && Angle >= -(180.0f - ForwardHalfAngle + AngleThreshold))
	{
		return EHodgeAlsMovementDirection::Left;
	}

	return EHodgeAlsMovementDirection::Backward;
}

bool UHodgeAlsMath::TryCalculatePoleVector(const FVector& ALocation, const FVector& BLocation, const FVector& CLocation,
