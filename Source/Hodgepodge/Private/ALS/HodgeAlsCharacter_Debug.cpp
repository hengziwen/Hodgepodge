...
#define LOCTEXT_NAMESPACE "HodgeAlsCharacterDebug"

void AHodgeAlsCharacter::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DisplayInfo, float& Unused, float& VerticalLocation)
{
	...
	if (!DisplayInfo.IsDisplayOn(UHodgeAlsConstants::CurvesDebugDisplayName()) &&
	    !DisplayInfo.IsDisplayOn(UHodgeAlsConstants::StateDebugDisplayName()) &&
	    !DisplayInfo.IsDisplayOn(UHodgeAlsConstants::ShapesDebugDisplayName()) &&
	    !DisplayInfo.IsDisplayOn(UHodgeAlsConstants::TracesDebugDisplayName()) &&
	    !DisplayInfo.IsDisplayOn(UHodgeAlsConstants::MantlingDebugDisplayName()))
	{
	...
	static const auto CurvesHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.Curves (Shift + 1)")})};
	...
	static const auto StateHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.State (Shift + 2)")})};

	if (DisplayInfo.IsDisplayOn(UHodgeAlsConstants::StateDebugDisplayName()))
	{
	...
	static const auto ShapesHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.Shapes (Shift + 3)")})};

	if (DisplayInfo.IsDisplayOn(UHodgeAlsConstants::ShapesDebugDisplayName()))
	{
	...
	static const auto TracesHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.Traces (Shift + 4)")})};

	if (DisplayInfo.IsDisplayOn(UHodgeAlsConstants::TracesDebugDisplayName()))
	{
	...
	static const auto MantlingHeaderText{FText::AsCultureInvariant(FString{TEXTVIEW("HodgeAls.Mantling (Shift + 5)")})};

	if (DisplayInfo.IsDisplayOn(UHodgeAlsConstants::MantlingDebugDisplayName()))
	{
...
void AHodgeAlsCharacter::DisplayDebugHeader(const UCanvas* Canvas, const FText& HeaderText, const FLinearColor& HeaderColor,
...
void AHodgeAlsCharacter::DisplayDebugCurves(const UCanvas* Canvas, const float Scale,
                                       const float HorizontalLocation, float& VerticalLocation) const
{
	...
	for (const auto& CurveName : CurveNames)
	{
		...
		Text.SetColor(FMath::Lerp(FLinearColor::Gray, FLinearColor::White, UHodgeAlsMath::Clamp01(CurveValue)));
...
void AHodgeAlsCharacter::DisplayDebugState(const UCanvas* Canvas, const float Scale,
                                      const float HorizontalLocation, float& VerticalLocation) const
{
	VerticalLocation += 4.0f * Scale;
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(ViewMode).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(LocomotionMode).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(FString{UHodgeAlsUtility::BoolToString(bDesiredAiming)}, false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(
		FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(DesiredRotationMode).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(RotationMode).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(DesiredStance).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(Stance).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(DesiredGait).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(Gait).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(OverlayMode).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});
	...
	Text.Text = FText::AsCultureInvariant(FName::NameToDisplayString(UHodgeAlsUtility::GetSimpleTagName(LocomotionAction).ToString(), false));
	Text.Draw(Canvas->Canvas, {HorizontalLocation + ColumnOffset, VerticalLocation});

	VerticalLocation += RowOffset;
}

void AHodgeAlsCharacter::DisplayDebugShapes(const UCanvas* Canvas, const float Scale,
                                       const float HorizontalLocation, float& VerticalLocation) const
{
	VerticalLocation += 4.0f * Scale;
	...
	static const auto ViewRotationText{
		FText::AsCultureInvariant(FName::NameToDisplayString(
			FString{GET_MEMBER_NAME_STRING_VIEW_CHECKED(FHodgeAlsViewState, Rotation)}, false))
	};
	...
	static const auto InputYawAngleText{
		FText::AsCultureInvariant(FName::NameToDisplayString(
			FString{GET_MEMBER_NAME_STRING_VIEW_CHECKED(FHodgeAlsLocomotionState, InputYawAngle)}, false))
	};
	...
	DrawDebugDirectionalArrow(GetWorld(),
	                          ...
	                          FeetLocation + FVector{0.0f, 0.0f, 3.0f} +
	                          UHodgeAlsVector::AngleToDirectionXY(LocomotionState.InputYawAngle) * 50.0f,
	...
	static const auto SpeedText{
		FText::AsCultureInvariant(FName::NameToDisplayString(
			FString{GET_MEMBER_NAME_STRING_VIEW_CHECKED(FHodgeAlsLocomotionState, Speed)}, false))
	};
	...
	static const auto VelocityYawAngleText{
		FText::AsCultureInvariant(FName::NameToDisplayString(
			FString{GET_MEMBER_NAME_STRING_VIEW_CHECKED(FHodgeAlsLocomotionState, VelocityYawAngle)}, false))
	};
	...
	DrawDebugDirectionalArrow(GetWorld(),
	                          FeetLocation,
	                          FeetLocation +
	                          UHodgeAlsVector::AngleToDirectionXY(LocomotionState.VelocityYawAngle) *
	...
	static const auto TargetYawAngleText{
		FText::AsCultureInvariant(FName::NameToDisplayString(
			FString{GET_MEMBER_NAME_STRING_VIEW_CHECKED(FHodgeAlsLocomotionState, SmoothTargetYawAngle)}, false))
	};
	...
	DrawDebugDirectionalArrow(GetWorld(),
	                          ...
	                          FeetLocation + FVector{0.0f, 0.0f, 6.0f} +
	                          UHodgeAlsVector::AngleToDirectionXY(LocomotionState.SmoothTargetYawAngle) * 50.0f,
	                          50.0f, Color.ToFColor(true), false, -1.0f, SDPG_World, 3.0f);
#endif
...
void AHodgeAlsCharacter::DisplayDebugTraces(const UCanvas* Canvas, const float Scale,
                                       const float HorizontalLocation, float& VerticalLocation) const
{
	VerticalLocation += 4.0f * Scale;
...
// ReSharper disable once CppMemberFunctionMayBeStatic
void AHodgeAlsCharacter::DisplayDebugMantling(const UCanvas* Canvas, const float Scale,
                                         const float HorizontalLocation, float& VerticalLocation) const
{
	VerticalLocation += 4.0f * Scale;
