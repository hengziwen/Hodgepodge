...
#include "HodgeAlsAnimNotifyState_EarlyBlendOut.generated.h"

UCLASS(DisplayName = "HodgeAls Early Blend Out Animation Notify State")
class HODGEPODGE_API UHodgeAlsAnimNotifyState_EarlyBlendOut : public UAnimNotifyState
{
	GENERATED_BODY()
...
protected:
	...
	FGameplayTag LocomotionModeEquals{HodgeAlsLocomotionModeTags::InAir};

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Settings", Meta = (EditCondition = "bCheckRotationMode"))
	FGameplayTag RotationModeEquals{HodgeAlsRotationModeTags::Aiming};

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Settings", Meta = (EditCondition = "bCheckStance"))
	FGameplayTag StanceEquals{HodgeAlsStanceTags::Crouching};

public:
	UHodgeAlsAnimNotifyState_EarlyBlendOut();
