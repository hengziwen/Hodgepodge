...
#include "HodgeAlsCameraComponent.generated.h"

class UHodgeAlsCameraSettings;
class ACharacter;

UCLASS(ClassGroup = "HODGEALS", Meta = (BlueprintSpawnableComponent),
...
class HODGEPODGE_API UHodgeAlsCameraComponent : public USkeletalMeshComponent
{
	GENERATED_BODY()

protected:
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Settings")
	TObjectPtr<UHodgeAlsCameraSettings> Settings;
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "HODGEALS|Camera")
	...
	UFUNCTION(BlueprintPure, Category = "HODGEALS|Camera", Meta = (ReturnDisplayName = "Camera Location"))
	FVector GetFirstPersonCameraLocation() const;

	UFUNCTION(BlueprintPure, Category = "HODGEALS|Camera", Meta = (ReturnDisplayName = "Pivot Location"))
	FVector GetThirdPersonPivotLocation() const;

	UFUNCTION(BlueprintPure, Category = "HODGEALS|Camera", Meta = (ReturnDisplayName = "Trace Start"))
	FVector GetThirdPersonTraceStartLocation() const;

	UFUNCTION(BlueprintPure, Category = "HODGEALS|Camera")
...
inline bool UHodgeAlsCameraComponent::IsFieldOfViewOverriden() const
{
	return bOverrideFieldOfView;
}

inline void UHodgeAlsCameraComponent::SetFieldOfViewOverriden(const bool bNewFieldOfViewOverriden)
{
	bOverrideFieldOfView = bNewFieldOfViewOverriden;
}

inline float UHodgeAlsCameraComponent::GetFieldOfViewOverride() const
{
	return FieldOfViewOverride;
}

inline void UHodgeAlsCameraComponent::SetFieldOfViewOverride(const float NewFieldOfView)
{
	FieldOfViewOverride = FMath::Clamp(NewFieldOfView, 5.0f, 175.0f);
}

inline float UHodgeAlsCameraComponent::GetPostProcessWeight() const
{
	return PostProcessWeight;
}

inline void UHodgeAlsCameraComponent::SetPostProcessWeight(const float NewPostProcessWeight)
{
	PostProcessWeight = UHodgeAlsMath::Clamp01(NewPostProcessWeight);
}

inline bool UHodgeAlsCameraComponent::IsRightShoulder() const
{
	return bRightShoulder;
}

inline void UHodgeAlsCameraComponent::SetRightShoulder(const bool bNewRightShoulder)
{
